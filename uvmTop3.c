...
void* compute_ouptput(void* arg){
   ...
   //sleep(1);
    ...
    counter_enable.counterTypeFlags =  UVM_COUNTER_NAME_FLAG_BYTES_XFER_HTD | UVM_COUNTER_NAME_FLAG_BYTES_XFER_DTH | UVM_COUNTER_NAME_FLAG_CPU_PAGE_FAULT_COUNT | UVM_COUNTER_NAME_FLAG_GPU_PAGE_FAULT_COUNT |
                                      UVM_COUNTER_NAME_FLAG_GPU_EVICTION_COUNT | UVM_COUNTER_NAME_FLAG_GPU_RESIDENT_COUNT | UVM_COUNTER_NAME_FLAG_CPU_RESIDENT_COUNT | UVM_COUNTER_NAME_FLAG_GPU_MEMORY_ALLOCATED |
                                      UVM_COUNTER_NAME_FLAG_OTHER_PROCESS_GPU_MEMORY_EVICTED | UVM_COUNTER_NAME_THRASHING_PAGES;
    ...
    for(;j>0;j=process->pid){
        ...
        for(int i=0;i<NVIDIA_MAX_PROCESSOR;i++){
            ...
            if(uuids[i].uuid[0] && !process->is_event_tracker_setup[i]){
                ...
                //printf("%lu,%d",event_tracker.rmStatus,sizeof(unsigned long));
               ...
               // printf("%lu,%d",counter_enable.rmStatus,sizeof(unsigned long));
...
fflush(stdout);
printf("| %-*s | %-*s | %-*s | %-*s | %-*s | %-*s | %-*s |\n",16,"Processor Id",16,"Number of Faults",16,"Evictions",16,"Resident Pages",32,"Physical Memory allocatd",39,"Memory Evicted of other Processes",16,"Thrashed Pages" );
fflush(stdout);

for(int j=0;j<NVIDIA_MAX_PROCESSOR;j++){
if(issued_params[i].params.is_event_tracker_setup[j]){
                if(j==0){  
                printf("| %-*lu | %-*lu | %-*lu | %-*lu | %-*lu | %-*lu | %-*lu |\n",16,j,16,*((unsigned long*)issued_params[i].params.counterbuffer[j]+2),16,0,16,*((unsigned long*)issued_params[i].params.counterbuffer[j]+12),32,*((unsigned long*)issued_params[i].params.counterbuffer[j]+13),39,*((unsigned long*)issued_params[i].params.counterbuffer[j]+14),16,*((unsigned long*)issued_params[i].params.counterbuffer[j]+15));
                fflush(stdout);
                }
                else{
                printf("| %-*lu | %-*lu | %-*lu | %-*lu | %-*lu | %-*lu | %-*lu |\n",16,j,16,*((unsigned long*)issued_params[i].params.counterbuffer[j]+9),16,*((unsigned long*)issued_params[i].params.counterbuffer[j]+10),16,*((unsigned long*)issued_params[i].params.counterbuffer[j]+11),32,*((unsigned long*)issued_params[i].params.counterbuffer[j]+13),39,*((unsigned long*)issued_params[i].params.counterbuffer[j]+14),16,*((unsigned long*)issued_params[i].params.counterbuffer[j]+15));
                fflush(stdout);
                }
                rows++;
                    //printf("%d,%llu\n",i,*((unsigned long*)counterbuffer[i]+13));
...
fflush(stdout);*/
//sleep(2);
pthread_create(&thread,NULL,compute_ouptput,(void*)params);
//sleep(2);
/*printf("=====================%d,%u==================================\n",i,uvm_pids[i]);
fflush(stdout);*/
...
if(issued_params[k].params.is_event_tracker_setup[j]){
     ...
     if(j==0){  
                printf("%d,%lu,%lu,%lu,%lu,%lu,%lu,%lu\n",issued_params[k].params.pid,j,*((unsigned long*)issued_params[k].params.counterbuffer[j]+2),0,*((unsigned long*)issued_params[k].params.counterbuffer[j]+12),*((unsigned long*)issued_params[k].params.counterbuffer[j]+13),*((unsigned long*)issued_params[k].params.counterbuffer[j]+14),*((unsigned long*)issued_params[k].params.counterbuffer[j]+15));
                }
                else{

                printf("%d,%lu,%lu,%lu,%lu,%lu,%lu,%lu\n",issued_params[k].params.pid,j,*((unsigned long*)issued_params[k].params.counterbuffer[j]+9),*((unsigned long*)issued_params[k].params.counterbuffer[j]+10),*((unsigned long*)issued_params[k].params.counterbuffer[j]+11),*((unsigned long*)issued_params[k].params.counterbuffer[j]+13),*((unsigned long*)issued_params[k].params.counterbuffer[j]+14),*((unsigned long*)issued_params[k].params.counterbuffer[j]+15));
                }
